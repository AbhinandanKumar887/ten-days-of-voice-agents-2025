...
def get_db_path() -> str:
    """Return absolute path for the DB file. If _file_ is not defined (interactive), fall back to cwd."""
    try:
        base = os.path.abspath(os.path.dirname(_file_))
    except NameError:
        base = os.getcwd()
...
class FoodAgent(Agent):
    def _init_(self):
        super()._init_(
            instructions="""
            You are 'rohit', a helpful assistant for 'abhinandan's', an Indian grocery store.
...
async def entrypoint(ctx: JobContext):
    ...
    logger.info("\n" + "🇮🇳" * 12)
    logger.info("🚀 STARTING ROHAN SHOP (Indian Context + Auto-Tracking)")
